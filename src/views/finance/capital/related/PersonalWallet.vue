  ...
  <div class="table-box">
    ...
    >
      ...
      <template #action="scope">
        ...
        <el-button type="primary" link :icon="Setting" @click="openDrawer('账户设置', scope.row)">账户设置</el-button>
...
const RuColumns = reactive<ColumnProps[]>([
  ...
  },
  {
    ...
    label: "销售金额",
    render(scope) {
      return convertFenToYuan(scope.row.contractUserName) || "--";
    }
  },
  { prop: "contractTelephone", label: "分润模式" },
  { prop: "contractTelephone", label: "分润费率" },
  {
    prop: "contractTelephone",
    label: "分润金额",
    render(scope) {
      return convertFenToYuan(scope.row.contractUserName) || "--";
    }
...
const TableRUJINRef = ref<ProTableInstance>();
const TableCHUJINRef = ref<ProTableInstance>();

const getTableList = (params: any) => {
  let newParams = JSON.parse(JSON.stringify(params));
  newParams.memberMerchantType = "person";
...
const openDrawer = (title: string, row: any = {}) => {
  const params = {
    ...
    isView: title === "查看",
    row: { ...row },
    api: title === "账户设置" ? undefined : title === "异常处置" ? undefined : undefined,
